...
# This script splits a large PDF into manageable chunks and uploads them to the RAG system

set -e

# Configuration
PDF_PATH="$1"
CHUNK_SIZE_PAGES=50  # Number of pages per chunk
MAX_FILE_SIZE_MB=5   # Maximum file size per chunk in MB
APP_URL="http://localhost:8080"
UPLOAD_DIR="./pdf_chunks"
...
print_status "Starting smart PDF splitting and upload process..."
...
if ! curl -s "$APP_URL/api/v1/health" > /dev/null 2>&1; then
    ...
    MAVEN_OPTS="-Xmx4g -Xms2g" mvn spring-boot:run > app.log 2>&1 &
    APP_PID=$!
